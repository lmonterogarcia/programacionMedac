...
public class B7_EJ36 {

	public static void main(String[] args) {
		float[] aVector = new float[100];
		float fNumeroN;
		
		aVector = leerVectorFloatHasta0("Introduzca un numero ", aVector, 0, 150);
//		System.out.println(imprimeVectorFloat(aVector));
		fNumeroN = (float)leer("Introduce el numero N1 ", -1, -1, 1, 150, (byte) 5);
		aVector = eliminarMenores(aVector, fNumeroN);
//		System.out.println(imprimeVectorFloat(aVector));
		fNumeroN = (float)leer("Introduce el numero N2 ", -1, -1, fNumeroN, 150, (byte) 5);
		aVector = introducirDetrarPrimerNumeroMenor(aVector, fNumeroN);
//		System.out.println(imprimeVectorFloat(aVector));
		fNumeroN = (float)leer("Introduce el numero N3 ", -1, -1, 1, 150, (byte) 5);
		aVector = introducirEnMedio(aVector, fNumeroN);
		
		System.out.println("\n"+imprimeVectorFloat(aVector));
	}
	...
	private static float[] eliminarMenores(float[] aVector, float fNumero) {
		byte bContador = 0;

		while (aVector[bContador] != 0) {

			if (aVector[bContador] < fNumero) {
				aVector[bContador] = 0;
				aVector = intercambiarPosicionesHaciaIzquierda(aVector, (byte) (bContador),
						ultimaPosicionLlena(aVector));
			} else {
				bContador++;
			}
		}
	...
	private static float[] introducirDetrarPrimerNumeroMenor(float[] aVector, float fNumero) {
		boolean booEsMenor = false;
		byte bContador = 0;
		
		while (!booEsMenor && aVector[bContador] != 0) {
			if (aVector[bContador] < fNumero ) {
				booEsMenor = true;
			}
		...
		if (booEsMenor) {
			aVector = intercambiarPosicionesHaciaDerecha(aVector, (byte) (ultimaPosicionLlena(aVector) + 1), (byte) (bContador + 1 ));
			aVector[bContador + 1] = fNumero;
		}
	...
	private static float[] introducirEnMedio(float[] aVector, float fNumero) {
		byte bPosicion;
		
		if (ultimaPosicionLlena(aVector) % 2 == 0) {
			bPosicion = (byte) ((ultimaPosicionLlena(aVector) / 2) +1);
		} else {
			bPosicion = (byte) ((Math.ceil(ultimaPosicionLlena(aVector) / 2)) + 1);
		}
		
		aVector = intercambiarPosicionesHaciaDerecha(aVector, (byte) (ultimaPosicionLlena(aVector) + 1), (byte) (bPosicion));
		aVector[bPosicion] = fNumero;
		
		return aVector;
	}
	
	private static float[] intercambiarPosicionesHaciaDerecha(float[] aVector, byte bPosicionUltimo, byte bPosicionHasta) {
		float bTemporal;

		for (byte bContador = bPosicionUltimo; bContador >= bPosicionHasta; bContador--) {
			bTemporal = aVector[bContador];
			aVector[bContador] = aVector[bContador + 1];
			aVector[bContador + 1] = bTemporal;
		}

		return aVector;
	}
	
	private static float[] intercambiarPosicionesHaciaIzquierda(float[] aVector, byte bDesde, byte bPosicionUltimo) {
		float bTemporal;

		for (byte bContador = bDesde; bContador <= bPosicionUltimo; bContador++) {
			bTemporal = aVector[bContador];
			aVector[bContador] = aVector[bContador + 1];
			aVector[bContador + 1] = bTemporal;
		}
	...
	private static byte ultimaPosicionLlena(float[] aVector) {
		byte bPosicion = (byte) (aVector.length - 1);
		boolean booLlena = false;

		while (!booLlena && bPosicion >= 0) {
			if (aVector[bPosicion] != 0) {
				booLlena = true;
			}
			if (!booLlena) {
				bPosicion--;
	...
	public static float[] leerVectorFloatHasta0(String sMensaje, float[] aVector, double dMin, double dMax) {
		int iContador = 0;
		
		do {
			aVector[iContador] = (float) leer(sMensaje + (iContador + 1) + ": ", -1, -1, dMin, dMax,(byte) 5);
			iContador++;
		} while (aVector[iContador - 1] != 0 && iContador < aVector.length);
