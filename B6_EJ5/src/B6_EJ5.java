...
public class B6_EJ5 {
	...
	public static void main(String[] args) {
		...
		final byte BCERCA = 10, BCERCAINTENTOS = 2;
		byte bIntentos = 3, bContadorIntentosCerca = 0;
		final short SHMIN = 50, SHMAX = 200, SHNUMERO = creacionNumeroAleatorio(SHMIN, SHMAX); 
		short shIntentoNumero;
		boolean booAcierta = false;
		
//		System.out.println(SHNUMERO);
		
		System.out.println("░▒▓██████████►╬◄███████████████▓▒░\n░▒▓██►╔╦╦╦═╦╗╔═╦═╦══╦═╗◄██▓▒░\n░▒▓██►║║║║╩╣╚╣═╣║║║║║╩╣◄██▓▒░\n░▒▓██►╚══╩═╩═╩═╩═╩╩╩╩═╝◄██▓▒░\n░▒▓██████████►╬◄███████████████▓▒░");
		System.out.println("\nHabia una vez un numero que estaba entre "+SHMIN+" y "+SHMAX+".\nTu mision es adivinar que numero es.\nLas reglas son:\n- Tienes 3 oportunidades.\n- Cuando introduzcas un numero, te dire si es mayor o menor.\n- Si introduces "+BCERCAINTENTOS+" veces un numero proximo en "+BCERCA+" numeros\nal numero que estas buscando, se te sumara un intento mas.\n\nEMPEZAMOS");

		do {
			shIntentoNumero = (short) leer("Introduce el numero que cree que es. Te quedan "+bIntentos+" intentos ", SHMIN, SHMAX, -1, -1, (byte) 2);
			if (shIntentoNumero == SHNUMERO) {
				booAcierta = true;
			} else {
	...
	private static short creacionNumeroAleatorio( short shMin, short shMax) {
		return (short) ((shMin) + Math.floor(Math.random() * (shMax - shMin) + 1));
	}
