...
//		short[] aPesosRatas = leerVector("Introduce el peso de la rata ", 20, 150, 450);
		short[] aPesosRatas = objectToShortVector(leerVector("Introduce el peso de la rata ", 20, 150, 450, -1, -1, (byte) 2));
//		short[] aPesosRatas = crearVectorRandomShort(20, (short) 150, (short) 450);
		System.out.println(informeRatas(aPesosRatas));
	}
	...
	private static String informeRatas(short[] aVector) {
		String sInformeRatas = "";
		byte bRepetiones = 1;

		aVector = intToShortVector(ordenarVectorBurbuja(shortToIntVector(aVector)));

		for (byte ContadorRatas = 0; ContadorRatas < aVector.length; ContadorRatas++) {
			if (ContadorRatas == aVector.length - 1) {
				if (aVector[ContadorRatas] == aVector[ContadorRatas - 1]) {
					sInformeRatas += "\nHay " + bRepetiones + " ratas que pesan " + aVector[ContadorRatas] + " gramos.";
				} else {
					sInformeRatas += "\nHay 1 rata que pesa " + aVector[ContadorRatas] + " gramos.";
				}
			} else {
				if (aVector[ContadorRatas] == aVector[ContadorRatas + 1]) {
					bRepetiones++;
					...
					if (bRepetiones > 1) {
						sInformeRatas += "\nHay " + bRepetiones + " ratas que pesan " + aVector[ContadorRatas]
								+ " gramos.";
					} else {
						sInformeRatas += "\nHay " + bRepetiones + " rata que pesa " + aVector[ContadorRatas]
								+ " gramos.";
					}
					bRepetiones = 1;
				}
		...
		sInformeRatas += "\nPeso medio: " + mediaDeVector(shortToIntVector(aVector)) + " gramos";
		return sInformeRatas;
	}

	private static float mediaDeVector(int[] aVector) {
		int iTotal = 0;
		for (int iContador = 0; iContador < aVector.length; iContador++) {
			iTotal += aVector[iContador];
		}
		return iTotal / aVector.length;
	}
