...
public class B7_EJ46 {

	public static void main(String[] args) {
		byte[][] aMatriz = {{6,0,0,0,4},{0,5,0,0,2},{2,0,0,0,0},{0,0,7,0,0},{0,0,0,8,0}};
		System.out.println(imprimeMatrizByte(aMatriz));
		byte[] aVectorUni = intToByteVector(crearMatrizUni(byteToIntMatriz(aMatriz), 5, 5));
		
		
		System.out.println(imprimeVectorInt(crearMatrizUni(byteToIntMatriz(aMatriz), 5, 5)));
//		System.out.println("\n"+imprimeMatrizInt(visualizar(byteToIntVector(aVectorUni), 5, 5)));
		System.out.println("\n"+visualizar2(byteToIntVector(aVectorUni), 5, 5));
	}
	...
	private static int[] crearMatrizUni(int[][] aMatriz, int iFilas, int iColumnas) {
		int[] aMatrizUni = new int[(elementosDiferentesDeCero(aMatriz) * 3)];
		int iContadorElementos = 0;
		
		for (int iContadorFilas = 0; iContadorFilas < iFilas ; iContadorFilas++) {
			for (int iContadorColumnas = 0; iContadorColumnas < iColumnas ; iContadorColumnas++) {
				
				if (aMatriz[iContadorFilas][iContadorColumnas] != 0) {
					aMatrizUni[iContadorElementos] = iContadorFilas;
					iContadorElementos++;
					aMatrizUni[iContadorElementos] = iContadorColumnas;
					iContadorElementos++;
					aMatrizUni[iContadorElementos] = aMatriz[iContadorFilas][iContadorColumnas];
					iContadorElementos++;
				}	
	...
	//A mi parecer seria mejor que el metodo visualizar() diese como resultado la matriz bidimensional, 
	//y asi poder hacer con ella lo que se quiera. En todo caso, el metdo visualuzar2() en vez de dar como
	//resultado un int[][] dara un String.
	private static int[][] visualizar(int[] aVector, int iFilas, int iColumnas){
		int[][] aMatriz = new int[iFilas][iColumnas];
		boolean booEstaEnVector;
		int iContadorElementosVector;
		
		for (int iContadorFilas = 0; iContadorFilas < iFilas; iContadorFilas++) {
			for (int iContadorColumnas = 0; iContadorColumnas < iColumnas; iContadorColumnas++) {
				booEstaEnVector = false;
				iContadorElementosVector = 0;
					do {
						
						if (aVector[iContadorElementosVector] == iContadorFilas &&  aVector[iContadorElementosVector + 1] == iContadorColumnas) {
							booEstaEnVector = true;
							
						}
						
						iContadorElementosVector += 3;
					} while (!booEstaEnVector && iContadorElementosVector < (aVector.length - 3));
					
					if (booEstaEnVector) {
						aMatriz[iContadorFilas][iContadorColumnas] = aVector[iContadorElementosVector - 1];
					} else {
						aMatriz[iContadorFilas][iContadorColumnas] = 0;
					}
	...
	private static String visualizar2(int[] aVector, int iFilas, int iColumnas){
		String sMatriz = "";
		boolean booEstaEnVector;
		int iContadorElementosVector;
		...
		for (int iContadorFilas = 0; iContadorFilas < iFilas; iContadorFilas++) {
			for (int iContadorColumnas = 0; iContadorColumnas < iColumnas; iContadorColumnas++) {
				...
				iContadorElementosVector = 0;
					...
					if (booEstaEnVector) {
						sMatriz += aVector[iContadorElementosVector - 1];
					} else {
						sMatriz += 0;
					}
					if(iContadorColumnas < iColumnas - 1) {
						sMatriz += " , ";
					}
	...
	private static int elementosDiferentesDeCero(int[][] aMatriz ) {
		int iElementos = 0;
		
		for (int iContadorFilas = 0; iContadorFilas < aMatriz.length; iContadorFilas++) {
			for (int iContadorColumnas = 0; iContadorColumnas < aMatriz[0].length; iContadorColumnas++) {
