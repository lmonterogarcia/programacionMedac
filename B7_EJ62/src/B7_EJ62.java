...
public class B7_EJ62 {

	public static void main(String[] args) {
		byte[][] aMatriz = {{1,3,1,1},{3,2,3,3},{1,3,1,1},{1,3,1,1}};
		
		System.out.println(esMatrizPar(aMatriz));
	}
	...
	private static char esMatrizPar(byte[][] aMatriz) {
		char cEsPar = '0';
		boolean booEsMenor = true, booEsPar = false;
		byte bFila = 0, bContadorColumna = 0;

		do {
			bFila = hayUnicoElementoMenorParPosicion(aMatriz, bContadorColumna);
			if (bFila != -1) {
				if (esUnicoPar(aMatriz, bFila, bContadorColumna)) {
					booEsPar = true;
				}
			}
			bContadorColumna++;
		} while (!booEsPar && bContadorColumna < aMatriz[0].length);
	...
	private static byte hayUnicoElementoMenorParPosicion(byte[][] aMatriz, byte bColumna) {
//		
//	Esta primera version la he desechado, y no la he terminado pq para una columna 2,2,3,2 petaria.	
//		byte bContadorFila1 = 0;
//		
//		for (int bContadorFila2 = 1; bContadorFila2 < aMatriz.length; bContadorFila2++) {
//			
//			if (aMatriz[bContadorFila1][bColumna] > aMatriz[bContadorFila2][bColumna]) {
//				bContadorFila1 = bContadorFila2;
//			} else if (aMatriz[bContadorFila1][bColumna] >= aMatriz[bContadorFila2][bColumna]) {
//				
//			}...
//			
//		}
...
//			
//		}
		byte[] aVectorColumna = columnaToVector(aMatriz, bColumna),
				aVectorPosiciones = crearVectorCreciente((byte) 0, (byte) (aMatriz.length - 1));
		byte bPosicionMenor = -1;

		aVectorPosiciones = ordenarVectorBurbujaConReferencia(aVectorColumna, aVectorPosiciones);
		aVectorColumna = ordenarVectorBurbuja(aVectorColumna);

		if ((aVectorColumna[0] != aVectorColumna[1]) && (aVectorColumna[0] % 2 == 0)) {
			bPosicionMenor = aVectorPosiciones[0];
		}

		return bPosicionMenor;
	}
	
	
	private static byte[] columnaToVector(byte[][] aMatriz, byte bColumna) {
		byte aVector[] = new byte[aMatriz.length];
		for (int iContador = 0; iContador < aVector.length; iContador++) {
			aVector[iContador] = aMatriz[iContador][bColumna];
		}
		return aVector;
	}
	
	private static byte[] crearVectorCreciente(byte bMinimo, byte bMaximo) {
		byte aVector[] = new byte[bMaximo + 1];
		for (int iContador = bMinimo; iContador <= bMaximo; iContador++) {
			aVector[iContador] = (byte) iContador;
		}
		return aVector;
	}

	private static boolean esUnicoPar(byte[][] aMatriz, byte bFila, byte bColumna) {
		boolean booEsUnico = true;
		byte bContadorColumna;
		
		if (bColumna == aMatriz.length - 1) {
			bContadorColumna = 0;
		} else {
			bContadorColumna = (byte) (bColumna + 1);
		}
		
		do {
			if (aMatriz[bFila][bContadorColumna] % 2 == 0) {
			booEsUnico = false;
		}
		
		if (bContadorColumna == aMatriz[0].length - 1) {
			bContadorColumna = 0;
		} else {
			bContadorColumna++;
		}
		} while (booEsUnico && bContadorColumna != bColumna );
	...
	private static byte[] ordenarVectorBurbuja(byte aVector[]) {
		double dTemporal;
		byte[] aVectorOrdenar = aVector;
		...
		for (iContadorI = 1; iContadorI < iElementos; iContadorI++) {
			for (iContadorJ = (iElementos - 1); iContadorJ >= iContadorI; iContadorJ--) {
				if (aVectorOrdenar[iContadorJ] < aVectorOrdenar[iContadorJ - 1]) {
					dTemporal = aVectorOrdenar[iContadorJ];
					aVectorOrdenar[iContadorJ] = aVectorOrdenar[iContadorJ - 1];
					aVectorOrdenar[iContadorJ - 1] = (byte) dTemporal;
				}
			}
		}
		return aVectorOrdenar;
	}
	
	private static byte[] ordenarVectorBurbujaConReferencia(byte[] aCodigo, byte[] aNota) {
		double dTemporal;
		byte[] aVectorReferencia = aCodigo;
		byte[] aVector = aNota;
		int iContadorI, iContadorJ, iElementos = aCodigo.length;

		for (iContadorI = 1; iContadorI < iElementos; iContadorI++) {
			for (iContadorJ = (iElementos - 1); iContadorJ >= iContadorI; iContadorJ--) {
				if (aCodigo[iContadorJ] < aCodigo[iContadorJ - 1]) {
				
					dTemporal = aCodigo[iContadorJ];
					aCodigo[iContadorJ] = aCodigo[iContadorJ - 1];
					aCodigo[iContadorJ - 1] = (byte) dTemporal;
					
					dTemporal = aVector[iContadorJ];
					aVector[iContadorJ] = aVector[iContadorJ - 1];
					aVector[iContadorJ - 1] = (byte) dTemporal;
				}
			}
		}
