...
public class B7_EJ56 {

	public static void main(String[] args) {
		float[][] aMatriz = crearMatrizRandomFloat(10, 8, 0, 100);
//		float[][] aMatriz ={{9,3},{8,8},{9,8},{9,18}};
		byte[] aVector = new byte[aMatriz.length];
		float fMediaMatriz = mediaMatriz(aMatriz), fDesviacionMatriz = desviacionMatriz(aMatriz, fMediaMatriz);
		
		
		System.out.println(imprimeMatrizFloat(aMatriz));
		System.out.println("\nMedia: "+fMediaMatriz+"\nDesviacion: "+fDesviacionMatriz);
		System.out.println("\n"+imprimeVectorByte(primerRepetidoEnCadaFila(aVector, aMatriz, fMediaMatriz, fDesviacionMatriz)));
	}
	...
	private static byte[] primerRepetidoEnCadaFila(byte[] aVector, float[][] aMatriz, float fMedia, float fDesviacion) {

		for (int iContadorFila = 0; iContadorFila < aMatriz.length; iContadorFila++) {
			
			if (dentroDeDesviacion(iContadorFila, aMatriz, fMedia, fDesviacion)) {
				aVector[iContadorFila] = 1;
			} else {
				aVector[iContadorFila] = 0;
			}
	...
	private static boolean dentroDeDesviacion(int iFila, float[][] aMatriz, float fMedia, float fDesviacion) {
		int iColumna = 0;
		boolean booSeRepite = false;
	
		do {
			
			if ((aMatriz[iFila][iColumna] >= (fMedia - fDesviacion)) && (aMatriz[iFila][iColumna] <= (fMedia + fDesviacion))) {
				booSeRepite = true;
			}
			
			iColumna++;
		
		} while (!booSeRepite && iColumna < aMatriz[0].length - 2);
		
		
		return booSeRepite;
	}
	
	private static float mediaMatriz(float[][] aMatriz) {
		float fMedia, fTotal = 0;
		...
		for (int iContadorFilas = 0; iContadorFilas < aMatriz.length; iContadorFilas++) {
			for (int iContadorColumnas = 0; iContadorColumnas < aMatriz[0].length; iContadorColumnas++) {
				fTotal += aMatriz[iContadorFilas][iContadorColumnas];
			}
		}
		
		fMedia = fTotal / (aMatriz.length * aMatriz[0].length);
		
		return fMedia;
	}
	
private static float desviacionMatriz(float[][] aMatriz, float fMediaMatriz) {
		float fDesviacion, fTotal = 0;
		
		for (int iContadorFilas = 0; iContadorFilas < aMatriz.length; iContadorFilas++) {
			for (int iContadorColumnas = 0; iContadorColumnas < aMatriz[0].length; iContadorColumnas++) {
				fTotal += Math.pow((aMatriz[iContadorFilas][iContadorColumnas] - fMediaMatriz), 2);
			}
		}
		
		fDesviacion = (float) Math.pow((fTotal / (aMatriz.length * aMatriz[0].length)), 0.5);
