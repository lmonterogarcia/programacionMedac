CREATE DEFINER=`fotografo`@`%` PROCEDURE `reservafotografo`.`producto_update`(IN oObject JSON)
BEGIN
	...
	ELSE
        ...
        IF vItems > 0 THEN 
            ...
            WHILE vIndex < vItems DO
                ...
                UPDATE Producto 
               	SET
               		precioProducto = `precioProductoParam`, 
               		costeProducto = `costeProductoParam`,
               		proveedorProducto = `ProveedorProductoParam`,
               		descripcionProducto = `descripcionProductoParam`
               	WHERE nombreProducto = `nombreProductoParam`;
