...
public class B7_EJ12 {

	public static void main(String[] args) {
		short shNumero;
		byte bcontador = 0; 
		final byte BCONTADORMAX= 50;
		
		do {
			shNumero = (short) leer("Introduce un numero ", -1000, 1000, -1, -1, (byte) 2);

			if (shNumero >= 0) {
				bcontador++;
				if (bcontador == BCONTADORMAX) {
					System.out.println("Ha llegado al limite del contador.");	
				}
			}

		} while (shNumero >= 0 && bcontador < BCONTADORMAX);

		if (bcontador > 1) {
			System.out.println("\nSe han introducido " + bcontador + " numeros.");
		} else {
			System.out.println("\nSe ha introducido " + bcontador + " numero.");
		}
	}
