...
public class B7_EJ42 {

	public static void main(String[] args) {
		String sMensaje = (leer("Introduce la frase", -1, 50, -1, -1, (byte) 10)).toString();
//		String sMensaje = "Las vocales son las letras que suenan como sí mismas sin la necesidad de incluir otro sonido vocal como es el caso de las consonantes, por esto mismo, en las palabras forman el núcleo de las sílabas, facilitando la pronunciación. Para comprender mejor las vocales, en la siguiente tabla se pueden observar las características de las vocales, incluyendo su escritura mayúscula, minúscula, ejemplos y su pronunciación.";
		System.out.println(imprimeVectorString(stringToStringVectorParejas(sMensaje)));

		System.out.println(
				"\n" + imprimeTablaFrecuencias(frecuenciaDeParejasEnString(stringToStringVectorParejas(sMensaje))));
	}
	...
	public static String imprimeTablaFrecuencias(short[][] aMatriz) {
		String sResultado = "    | a | b | c | d | e | f  | g | h | i  | j  | k |  l | m | n | ñ | o | p | q | r | s | t  | u | v | w | x | y | z | #\n------------------------------------------------------------------------------------------------------------------\n";
		for (int iContadorFila = 0; iContadorFila < aMatriz.length; iContadorFila++) {
		
			sResultado += " "+NumeroToLetra((byte) (iContadorFila))+" | ";
	...
	//No esta contemplado letras con acento o la ç. Esas letras seran NULL (#).
	private static short[][] frecuenciaDeParejasEnString(String[] aMensajeEnParejas) {
		byte bLetras = 27 + 1;
		short[][] aFrecuencias = new short [bLetras][bLetras];
//		String sPareja;
		
		for (int iContador = 0; iContador < aMensajeEnParejas.length; iContador++) {

			aFrecuencias[letraToNumero((aMensajeEnParejas[iContador]).charAt(0))][letraToNumero((aMensajeEnParejas[iContador]).charAt(1))]++; 
		}
	...
	private static byte letraToNumero(char cLetra) {
		byte bNumero, bContador = 0;
		char[] aAlfabeto = {'a','b','c','d','e','f','g','h','i','j','k','l','m','n','ñ','o','p','q','r','s','t','u','v','w','x','y','z','#'};
		boolean booEsLaLetra = false;
		
		do {
			if (cLetra == aAlfabeto[bContador]) {
				booEsLaLetra = true;
			}
			bContador++;
		} while (!booEsLaLetra && bContador < aAlfabeto.length);
		
		if (!booEsLaLetra) {
			bNumero = (byte) (aAlfabeto.length - 1);
		} else {
			bNumero = (byte) (bContador - 1);
		}
		return bNumero;
	}  
	
	private static char NumeroToLetra(byte bNumero) {
		char cLetra;
		char[] aAlfabeto = {'a','b','c','d','e','f','g','h','i','j','k','l','m','n','ñ','o','p','q','r','s','t','u','v','w','x','y','z','#'};
		
		cLetra = aAlfabeto[bNumero];
		
		
		return cLetra;
	}  
	
	private static String[] stringToStringVectorParejas(String sMensaje) {
		String[] aPorParejas = new String[(soloParejasDeLetrasAdyacentes(sMensaje))];
		int iContadorElementosVector = 0;
		String sTodoMinusculas = mayusToMinus(sMensaje);
		
		for (int iContador = 0; iContador < sMensaje.length() - 1; iContador++) {
			if (Character.isLetter(sMensaje.charAt(iContador)) && Character.isLetter(sMensaje.charAt(iContador + 1))) {
				aPorParejas[iContadorElementosVector] = sTodoMinusculas.substring(iContador, iContador + 2);
				iContadorElementosVector++;
			}
		}
	...
	private static int soloParejasDeLetrasAdyacentes(String sMensaje) {
		int iContadorDeParejas = 0;

		for (int iContador = 0; iContador < sMensaje.length() - 1; iContador++) {
//			if (sMensaje.charAt(iContador) != ' ' && sMensaje.charAt(iContador + 1) != ' ') {
			if (Character.isLetter(sMensaje.charAt(iContador)) && Character.isLetter(sMensaje.charAt(iContador + 1))) {
				iContadorDeParejas ++;
			}
		}
		return iContadorDeParejas;
	}
	
	private static String mayusToMinus(String sMensaje) {
		String sMensajeResultado = "";
		
		for (int iContador = 0; iContador < sMensaje.length(); iContador++) {
			if ((sMensaje.charAt(iContador) >= 'A' && sMensaje.charAt(iContador) <= 'Z') || sMensaje.charAt(iContador) == 'Ñ') {
				sMensajeResultado += Character.toLowerCase(sMensaje.charAt(iContador));		
			} else {
				sMensajeResultado += sMensaje.charAt(iContador);	
			}
		}
		return sMensajeResultado;
	}
	...
	private static int[] ordenarVectorBurbuja(int aVector[]) {
		double dTemporal;
		int iContadorI, iContadorJ, iElementos = aVector.length;

		for (iContadorI = 1; iContadorI < iElementos; iContadorI++) {
			for (iContadorJ = (iElementos - 1); iContadorJ >= iContadorI; iContadorJ--) {
				if (aVector[iContadorJ] < aVector[iContadorJ - 1]) {
					dTemporal = aVector[iContadorJ];
					aVector[iContadorJ] = aVector[iContadorJ - 1];
					aVector[iContadorJ - 1] = (int) dTemporal;
				}
			}
		}
		return aVector;
	...
	public static int posicionNumeroMax(int[] aVector) {
		int iPosicionValorMax = 0;
		for (int iValorControl = 1; iValorControl < aVector.length; iValorControl++) {
			if (aVector[iPosicionValorMax] < aVector[iValorControl]) {
				iPosicionValorMax = iValorControl;
			}
		}
		return iPosicionValorMax;
	}
	
	private static int[] moverPosicionesVector(int[] aVector, int iPosiciones) {
		double iIntercambio;
		
		for (int iContador1 = 0; iContador1 < iPosiciones; iContador1++) {
			iIntercambio = aVector[aVector.length - 1];
			for (int iContador2 = aVector.length - 1; iContador2 > 0; iContador2--) {
				double dTemporal = aVector[iContador2];
				aVector[iContador2] = aVector[iContador2 - 1];
				aVector[iContador2 - 1] = (int) dTemporal;
			}
			aVector[0] = (int) iIntercambio;
		}
		iIntercambio = aVector[aVector.length - 1];
		for (int iContador2 = aVector.length - 2; iContador2 >= 0; iContador2--) {
	...
	private static boolean esMatrizSimetrica(int[][] aMatriz) {
		boolean booEsSimetrica = true;
		int iContadorI = 0, iContadorJ = aMatriz.length - 1;

		if (aMatriz.length == aMatriz[0].length) {
			
			while (booEsSimetrica && iContadorI < aMatriz.length - 1) {

				if (aMatriz[iContadorI][iContadorJ] != aMatriz[iContadorJ][iContadorI]) {
					booEsSimetrica = false;		
				}
				
				iContadorJ--;

				if (booEsSimetrica && (iContadorJ == iContadorI)) {
					iContadorI++;
					iContadorJ = aMatriz.length - 1;
				}
				
			}
			
		} else {
			System.err.print("La Matriz no es cuadrada. ");
			booEsSimetrica = false;
		}

		return booEsSimetrica;
	}
	
	private static int[][] crearMatrizEnteroRandom(int iFilas, int iColumnas, int iMin, int iMax) {
	...
	private static int sumaFila(int[][] aMatriz, int iFila) {
		int iSumaFila = 0;
		for (int iContadorColumnas = 0; iContadorColumnas < aMatriz[iFila].length; iContadorColumnas++) {
			iSumaFila += aMatriz[iFila][iContadorColumnas];
		}
		return iSumaFila;
	}
	...
	private static String sumaCadaFila(int[][] aMatriz) {
		...
		for (int iContador = 0; iContador < aMatriz.length; iContador++) {
			sSumaCadaFila += "\nLa suma de la fila " + iContador + " es " + sumaFila(aMatriz, iContador);
		}
		return sSumaCadaFila;
	}

	private static int sumaColumna(int[][] aMatriz, int iColumna) {
		int iSumaColumna = 0;
		for (int iContadorColumna = 0; iContadorColumna < aMatriz.length; iContadorColumna++) {
			iSumaColumna += aMatriz[iContadorColumna][iColumna];
		}
		return iSumaColumna;
	}
	...
	private static String sumaCadaColumnas(int[][] aMatriz) {
		...
		for (int iContador = 0; iContador < aMatriz[0].length; iContador++) {
			sSumaCadaColumna += "\nLa suma de la columna " + iContador + " es " + sumaColumna(aMatriz, iContador);
		}
		return sSumaCadaColumna;
	}

	private static int sumaMatriz(int[][] aMatriz) {
		int iSumaMatriz = 0;
		for (int iContadorFilas = 0; iContadorFilas < aMatriz.length; iContadorFilas++) {
			for (int iContadorColumnas = 0; iContadorColumnas < aMatriz[iContadorFilas].length; iContadorColumnas++) {
				iSumaMatriz += aMatriz[iContadorFilas][iContadorColumnas];
			}
		}
		return iSumaMatriz;
	}
