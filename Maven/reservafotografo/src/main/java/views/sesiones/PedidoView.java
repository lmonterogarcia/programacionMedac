...
public class PedidoView implements IPlantilla{
    ...
    public static byte subMenuPedido() {
		System.out.println("");
		System.out.println("------------------");
					...
					if (lPedidos != null && !lPedidos.isEmpty()) {
						...
						for (Pedido oPedL : lPedidos) {
							System.out.println(" Id de pedido: " + oPedL.getiIdPedido() + " - Empresa: "
									+ oPedL.getoEmpresa().getsNombreEmpresa() + " - Pagado: " + (oPedL.isBooPagado() ? "Pagado" : "NO PAGADO"));
						}
					} else {
	...
	private static boolean update(Controller oCtrl) {
		...
		if (oPedido != null) {
			if (oPedido != null && oPedido.checkPedido()) {
				...
				do {
					sLetra = String
							.valueOf(Libreria.leer("¿Ha sido pagado?(s/n) (" + (oPedido.isBooPagado() == true ? "pagado" : "no pagado") + ")",
									1, 1, -1, -1, (byte) 6));
	...
	private static Pedido searchById(Controller oCtrl) {
		...
		return oCtrl.getSesionesCtrl().getoPedidoCtrl().searchByPk(new Pedido(iIdPedido));
	}
	...
	public static List<Pedido> listarPedido(Controller oCtrl) {
		return oCtrl.getSesionesCtrl().getoPedidoCtrl().listar();
	}
}
