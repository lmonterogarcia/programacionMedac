CREATE DEFINER=`fotografo`@`%` PROCEDURE `reservafotografo`.`producto_sesion_search_by`(IN oObject JSON)
BEGIN
        ...
        IF vItems > 0 THEN 
            WHILE vIndex < vItems DO
                ...
                SET idSesionParam = JSON_UNQUOTE(JSON_EXTRACT(oObject, CONCAT('$[', vIndex, '].iIdSesion')));
               	...
               	SELECT * FROM Producto_Sesion WHERE nombreProducto = nombreProductoParam AND idSesion = idSesionParam;
