...
public class B6_EJ6 {
	...
	public static void main(String[] args) {
		char[] aPalabra = palabraRandom(), aGuiones = filaGuiones(aPalabra), aUsadas = new char[aPalabra.length + 5];
		char cCaracter;
		byte bIntentos = 6, bJugadas = 0;
		boolean booPalabra = false;
		boolean[] aCaracteres = new boolean[aPalabra.length];

		System.out.println(
				"░▒▓██████████►╬◄███████████████▓▒░\n░▒▓██►╔╦╦╦═╦╗╔═╦═╦══╦═╗◄██▓▒░\n░▒▓██►║║║║╩╣╚╣═╣║║║║║╩╣◄██▓▒░\n░▒▓██►╚══╩═╩═╩═╩═╩╩╩╩═╝◄██▓▒░\n░▒▓██████████►╬◄███████████████▓▒░\n\nJUEGO DEL AHORCADO\n\nLa palabra tiene "+aPalabra.length+" letras.\n");
	
		do {
			
			if (bIntentos > 1) {
				System.out.println(imprimeCaracterSinComas(aGuiones) + "\n\n" + imagenAhorcado((byte) (6 - bIntentos))
				+ "\nTe quedan " + bIntentos + " intentos.");
			} else {
				System.out.println(imprimeCaracterSinComas(aGuiones) + "\n\n" + imagenAhorcado((byte) (6 - bIntentos))
				+ "\nTe queda " + bIntentos + " intento.");
			}
	
			if (bJugadas > 0) {
				if (bJugadas > 1) {
					System.out.println("Has utilizado las siguientes letras: "+imprimeCaracterSinComas(aUsadas));
				} else {
					System.out.println("Has utilizado la siguiente letra: "+imprimeCaracterSinComas(aUsadas));
				}
			}
			
			cCaracter = leerLetra("Introduce una letra (minuscula):", "Solo se admiten letras minusculas y solo una, intentelo de nuevo", 'a', 'z');
		
			if (estaLaLetra(cCaracter, aUsadas)) {
				System.err.println("\nEsa letra ya ha sido utilizada, pruebe otra\n");
			} else {
				if (estaLaLetra(cCaracter, aPalabra)) {
					System.out.println("\n\nLa letra esta en la palabra");
					aGuiones = reemplazarGuiones(cCaracter, aPalabra, aGuiones);
					aCaracteres = reemplazarFalsos(cCaracter, aPalabra, aCaracteres);
					aUsadas[bJugadas] = cCaracter;
					bJugadas++;
					if (palabraCorrecta(aCaracteres)) {
						booPalabra = true;
					}
				} else {
		...
		} while (!booPalabra && bIntentos > 0);
		
		System.out.println(mostrarMensajeFinal(booPalabra, aPalabra));
	}
	...
	private static String mostrarMensajeFinal(boolean booPalabra, char[] aPalabra) {
		String sMensajeFinal, sPalabra = imprimePalabra(aPalabra);
		
		if (booPalabra) {
			sMensajeFinal = 
					"\nHas acertado, la palabra es \""+sPalabra+"\". Enhorabuena!!\n\n                                     YOU WIN\n\n________´$$$$`_____________________________,,,_\n_______´$$$$$$$`_________________________´$$$`\n________`$$$$$$$`______,,________,,_______´$$$$´\n_________`$$$$$$$`____´$$`_____´$$`____´$$$$$´\n__________`$$$$$$$`_´$$$$$`_´$$$$$`__´$$$$$$$´\n___________`$$$$$$$_$$$$$$$_$$$$$$$_´$$$$$$$´_\n____________`$$$$$$_$$$$$$$_$$$$$$$`´$$$$$$´_\n___,,,,,,______`$$$$$$_$$$$$$$_$$$$$$$_$$$$$$´_\n_´$$$$$`____`$$$$$$_$$$$$$$_$$$$$$$_$$$$$$´_\n´$$$$$$$$$`´$$$$$$$_$$$$$$$_$$$$$$$_$$$$$´_\n´$$$$$$$$$$$$$$$$$$_$$$$$$$_$$$$$$$_$$$$$´_\n___`$$$$$$$$$$$$$$$_$$$$$$$_$$$$$$_$$$$$$´_\n______`$$$$$$$$$$$$$_$$$$$__$$$$$_$$$$$´_\n_______`$$$$$$$$$$$$,___,$$$$,____,$$$$$$´_\n_________`$$$$$$$$$$$$$$$$$$$$$$$$$$$$$´_\n__________`$$$$$$$$$$$$$$$$$$$$$$$$$$$´_\n____________`$$$$$$$$$$$$$$$$$$$$$$$$´_\n_______________`$$$$$$$$$$$$$$$$$$$$´_";
		} else {
			sMensajeFinal = "Has superado el numero de intentos.\nLa palabra era \""+sPalabra+"\".\n\n"+imagenAhorcado((byte) (6))+"\nGAME OVER";
		}
		
		return sMensajeFinal;
	} 
	
	private static boolean palabraCorrecta(boolean[] aCaracteres) {
		boolean booEsCorrecta = true;
		byte bContador = (byte) (aCaracteres.length - 1);

		while (booEsCorrecta && bContador > 0) {
			if (aCaracteres[bContador] == false) {
				booEsCorrecta = false;
			}
			bContador--;
		}

		return booEsCorrecta;
	}

	private static boolean[] reemplazarFalsos(char cLetra, char[] aPalabra, boolean[] aCaracteres) {

		for (byte bContador = 0; bContador < aCaracteres.length; bContador++) {
			if (aPalabra[bContador] == cLetra) {
				aCaracteres[bContador] = true;
			}
		}
	...
	private static char[] reemplazarGuiones(char cLetra, char[] aPalabra, char[] aGuiones) {

		for (byte bContador = 0; bContador < aGuiones.length; bContador++) {
			if (aPalabra[bContador] == cLetra) {
				aGuiones[bContador] = cLetra;
			}
		}
	...
	private static boolean estaLaLetra(char cLetra, char[] aPalabra) {
		boolean booEstaLaLetra = false;
		byte bContador = (byte) (aPalabra.length - 1);

		do {
			if (cLetra == aPalabra[bContador]) {
				booEstaLaLetra = true;
			}
			bContador--;
		} while (!booEstaLaLetra && bContador >= 0);

		return booEstaLaLetra;
	}

	private static char leerLetra(String sMensaje, String sMensajeError, char cMin, char cMax) {
		...
		do {
			...
			if ((sCadenaLeida.length() != 1) || (cCaracter < cMin || cCaracter > cMax)) {
				System.err.println(sMensajeError);
			}
		} while ((!booCorrecto || sCadenaLeida.length() != 1) || (cCaracter < cMin || cCaracter > cMax));

		return cCaracter;
	}

	private static char[] filaGuiones(char[] aVector) {
		char[] aVectorGuiones = new char[aVector.length];
		for (int iContador = 0; iContador < aVector.length; iContador++) {
			aVectorGuiones[iContador] = '_';
		}

		return aVectorGuiones;
	}

	public static String imprimeCaracterSinComas(char[] aVector) {
		String sResultado = "";
		for (int iContador = 0; iContador < aVector.length; iContador++) {
	...
	public static String imprimePalabra(char[] aPalabra) {
		String sResultado = "";
		for (int iContador = 0; iContador < aPalabra.length; iContador++) {
			sResultado += aPalabra[iContador];
		}
		return sResultado;
	}

	private static String imagenAhorcado(byte bError) {
	...
	private static char[] palabraRandom() {
		String[] aPalabras = {"adaptacion","agudo","antibiotico","arritis","ataxia","atrofia","circulacion","cirugia","conducta","contractura","cronico","excremento","fisico","funcional","herencia","infantil","coche","rascacielos","elefante","leon","antilope","ballena","cabaña","acacia","pino","acebuche","persona","gente","bebe","rodilla","metacarpo","boca","labio","nariz","amor","matrimonio","nacimiento","muerte","naturaleza","esternocleidomastoideo","bosque","campo","selva","perro"};
		byte bPosicion = (byte) (Math.floor(Math.random() * (aPalabras.length - 1)));
		String sPalabra = aPalabras[bPosicion];
		char[] aPalabra = new char[sPalabra.length()];
		
		for (byte bContador = 0; bContador < aPalabra.length; bContador++) {
			aPalabra[bContador] = sPalabra.charAt(bContador);
		}
