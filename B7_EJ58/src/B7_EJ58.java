...
public class B7_EJ58 {

	public static void main(String[] args) {
	byte[][] aMatriz = crearMatrizRandomByte(25, 10, (byte) 1, (byte) 9);	
	byte[] aVector = new byte[aMatriz.length];
	aVector = todoVectorUnValor(aVector, (byte) -1);
	
	aVector = primerRepetidoEnCadaFila(aVector, aMatriz, (byte) -1);
	...
	private static byte[] primerRepetidoEnCadaFila(byte[] aVector, byte[][] aMatriz, byte bValorSINoEncuentra) {
		int iColumna;
		for (int iContadorFila = 0; iContadorFila < aMatriz.length; iContadorFila++) {
			
			iColumna = hayRepetidosConsecutivosEnFila(iContadorFila, aMatriz);
			
			if (iColumna >= 0) {
				aVector[iContadorFila] = aMatriz[iContadorFila][iColumna];
			} else {
				aVector[iContadorFila] = bValorSINoEncuentra;
			}
	...
	private static int hayRepetidosConsecutivosEnFila(int iFila, byte[][] aMatriz) {
		int iColumna = 0;
		boolean booSeRepite = false;
		
		while (!booSeRepite && iColumna < aMatriz[0].length - 2) {
			if (aMatriz[iFila][iColumna] == aMatriz[iFila][iColumna + 1]) {
				booSeRepite = true;
			}
			iColumna++;
		}
	...
	private static byte[] todoVectorUnValor(byte[] aVector, byte bNumero) {
		
		for (int iContador = 0; iContador < aVector.length; iContador++) {
			aVector[iContador] = bNumero;
		}
