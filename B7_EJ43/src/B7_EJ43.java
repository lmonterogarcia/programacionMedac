...
public class B7_EJ43 {

	public static void main(String[] args) {
//		byte[] aPesos = crearVectorRandomByte(50, (byte)10, (byte)100);
		byte[] aPesos = {10,20,10,60,50,80,30,30,30,60,90,20,20,20,40,40,40,80,50,50,10,10,70,70};
		
		aPesos = ordenarVectorBurbuja(aPesos);
		
		System.out.println(informe(aPesos));
	...
	private static String informe(byte[] aVector) {
		String sMensaje = "Peso  Numero de alumnos\n-------------------------------\n";
		byte bContadorPeso = 1;

		for (byte bContador = 0; bContador < aVector.length; bContador++) {
			if (bContador != aVector.length - 1) {
				if (aVector[bContador] == aVector[bContador + 1]) {
					bContadorPeso++;
				} else {
					sMensaje += aVector[bContador] + "     ";
					for (byte bContadorAsterisco = 1; bContadorAsterisco <= bContadorPeso; bContadorAsterisco++) {
						sMensaje += "*";
					}
					sMensaje += "\n";
					bContadorPeso = 1;
