...
public class B6_EJ1 {
	...
	public static void main(String[] args) {
		...
		final byte BDEPARTAMENTOS = 5, BSATISFACCIONMIN = 50, BSATISFACCIONMAX = 100;
		final int 	ISUELDOMIN = 18000, ISUELDOMAX = 43000;
		byte bDepartamento = (byte) leer("Introduce el numero del departamento ", 1, BDEPARTAMENTOS, -1, -1, (byte) 1);
		
//		int[][] aGestionEmpleados = creacionMatrizEmpleados((short) 100, BDEPARTAMENTOS, ISUELDOMIN, ISUELDOMAX, BSATISFACCIONMIN, BSATISFACCIONMAX);
//		System.out.println(imprimeMatrizInt(aGestionEmpleados));
//		System.out.println(informeDepartamento(aGestionEmpleados, bDepartamento));
		
		System.out.println(informeDepartamento(creacionMatrizEmpleados((short) 100, BDEPARTAMENTOS, ISUELDOMIN, ISUELDOMAX, BSATISFACCIONMIN, BSATISFACCIONMAX), bDepartamento));
	}
	...
	private static String informeDepartamento(int[][] aMatriz, byte bDepartamento) {
		long lSumaSalario = 0;
		short shSumaSatisfaccion = 0, shEmpleadosDepartamento = 0;
		
		for (int iContadorEmpleado = 0; iContadorEmpleado < aMatriz.length; iContadorEmpleado++) {
			if (aMatriz[iContadorEmpleado][0] == bDepartamento) {
				lSumaSalario += aMatriz[iContadorEmpleado][1];
				shSumaSatisfaccion += aMatriz[iContadorEmpleado][2];
				shEmpleadosDepartamento++;
			}
		}
		
		return "\nHay "+shEmpleadosDepartamento+" empleados en el departamento.\nEl salario medio es "+((float)lSumaSalario / shEmpleadosDepartamento)+"€.\nLa satisfaccion media es "+((float)shSumaSatisfaccion / shEmpleadosDepartamento)+"%.";
				
	}
	
	
	private static int[][] creacionMatrizEmpleados(short shNumeroEmpleados, byte bNumeroDepartamentos, int iSueldoMin, int SueldoMax, byte iSatisfaccionMin, byte bSatisfaccionMax){
		int[][] aEmpleados = new int[shNumeroEmpleados][3];
		
		for (int iContadorEmpleado = 0; iContadorEmpleado < aEmpleados.length; iContadorEmpleado++) {
			for (int iContadorColumna = 0; iContadorColumna < aEmpleados[iContadorEmpleado].length; iContadorColumna++) {
				if (iContadorColumna == 0) {
					aEmpleados[iContadorEmpleado][iContadorColumna] =  (int) (Math.floor(Math.random() * ((bNumeroDepartamentos) + 1))) ;
				} else if (iContadorColumna == 1) {
					aEmpleados[iContadorEmpleado][iContadorColumna] =  (int) ((iSueldoMin) + Math.floor(Math.random() * ((SueldoMax -iSueldoMin) + 1))) ;
				} else {
					aEmpleados[iContadorEmpleado][iContadorColumna] =  (int) ((iSatisfaccionMin) + Math.floor(Math.random() * ((bSatisfaccionMax - iSatisfaccionMin) + 1)));
				}
			}
		}
