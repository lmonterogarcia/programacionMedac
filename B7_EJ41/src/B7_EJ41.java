...
public class B7_EJ41 {
	...
	public static void main(String[] args) {
		...
		System.out.println(imprimeVectorInt(ordenarVectorBurbujaParImpar(byteToIntVector(aVector))));
	}
	...
	private static int[] ordenarVectorBurbujaParImpar(int aVector[]) {
		double dTemporal;
		...
		for (iContadorI = 2; iContadorI < iElementos; iContadorI += 2) {
 			for (iContadorJ = (iElementos + bPar); iContadorJ >= iContadorI; iContadorJ -= 2) {
				if (aVector[iContadorJ] < aVector[iContadorJ - 2]) {
					dTemporal = aVector[iContadorJ];
					aVector[iContadorJ] = aVector[iContadorJ - 2];
					aVector[iContadorJ - 2] = (int) dTemporal;
				}
			}
		}
		
		for (iContadorI = 3; iContadorI < iElementos; iContadorI += 2) {
			for (iContadorJ = (iElementos + bImpar); iContadorJ >= iContadorI; iContadorJ -= 2) {
				if (aVector[iContadorJ] > aVector[iContadorJ - 2]) {
					dTemporal = aVector[iContadorJ];
					aVector[iContadorJ] = aVector[iContadorJ - 2];
					aVector[iContadorJ - 2] = (int) dTemporal;
