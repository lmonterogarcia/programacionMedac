...
public class B5_EJ3 {
	...
	public static void main(String[] args) {
		byte[] aBoleto = leerCombinacion("Intruduce los numeros de tu boleto, numero ", (byte) 6, (byte) 1,(byte) 49),
				aCombinacionGanadora = leerCombinacion("Intruduce los numeros de la combinacion ganadora, numero ", (byte) 6, (byte) 1, (byte) 49);
		byte bAciertos = 0;

		for (int iContador = 0; iContador <= aCombinacionGanadora.length - 1; iContador++) {
			if (aBoleto[iContador] == aCombinacionGanadora[iContador]) {
				bAciertos++;
			}
		}
		
		System.out.println("\nSi la combinacion Ganadora es " + imprimeBoleto(aCombinacionGanadora) + " y el boleto "
				+ imprimeBoleto(aBoleto) + ", tiene " + bAciertos + " aciertos.");
	}

	public static byte[] leerCombinacion(String sMensaje, byte bTamanio, byte bMin, byte bMax) {

		byte bNumeroElementos = bTamanio, iPosicion;
		byte[] aVector = new byte[bNumeroElementos];
		boolean booEstaRepetido;

		for (byte bContador = 0; bContador < aVector.length; bContador++) {
			do {
				booEstaRepetido = false;
				iPosicion = (byte) (bContador - 1);
				aVector[bContador] = (byte) leer(sMensaje + (bContador + 1) + ": ", (long) bMin, (long) bMax, -1, -1,
						(byte) 1);
				while (!booEstaRepetido && iPosicion >= 0) {
					if (aVector[bContador] == aVector[iPosicion]) {
						booEstaRepetido = true;
					}
					iPosicion--;
				}
			} while (booEstaRepetido);
		}
		
		return aVector;
	}

	public static String imprimeBoleto(byte[] aVector) {
		String sResultado = "";
...
//private static boolean estaRepetido (int[] aVector, int iPosicion) {
//boolean booEstaRepetido = false;
//int iContador = 0;
//while (!booEstaRepetido || iContador <= aVector.length -1) {
//	if (aVector[iContador] == aVector[iPosicion]) {
//		booEstaRepetido = true;
//	}
//	iContador++;
//}
//return booEstaRepetido;
//}

//	public static int[] leerVectorIncrementoSinRepeticion(String sMensaje, int iTamanio, int iMin, int iMax) {
//
//		int iNumeroElementos = iTamanio, iControl = iMin - 1;
//		int[] aVector = new int[iNumeroElementos];
//
//		for (int iContador = 0; iContador < aVector.length; iContador++) {
//			do {
//				aVector[iContador] = (int) leer(sMensaje + (iContador + 1) + ": ", (long) iControl + 1, (long) iMax, -1, -1,
//						(byte) 3);
//			} while (estaRepetido(aVector, iContador));
//			
//			if (aVector[iContador] == iMax) {
//				System.err.println(
//						"\nHa tenido un error al introducir los datos. Ha llegado maximo y le quedan datos por introducir. Se empieza de nuevo.");
//				iContador = -1;
//				iControl = iMin - 1;
//			} else {
//				iControl = aVector[iContador];
//			}
//
//		}
//
