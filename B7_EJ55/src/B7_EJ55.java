...
public class B7_EJ55 {

	public static void main(String[] args) {
		float[] aVector = crearVectorRandomFloat(10, 0, 9);
		float fModulo = moduloVector(aVector);

		System.out.println(imprimeVectorFloat(aVector)+"\n\nEl modulo del vector es "+fModulo);
	}
	...
	private static float moduloVector(float[]aVector) {
		float fModulo, fTotal = 0;
		
		for (int iContador = 0; iContador < aVector.length; iContador++) {
			fTotal += Math.pow(aVector[iContador], 2);
		}
		
		fModulo = (float) Math.pow(fTotal, 0.5);
