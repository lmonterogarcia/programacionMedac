...
public class B7_EJ54 {

	// El cero, por ser un concepto numérico especial, no se incluía en el conjunto
	// de los números naturales ℕ, por convenio.
	public static void main(String[] args) {
		
		System.out.println("\nPotencia de 2");
		if (validacionFermat(2, 1, 100)) {
			System.out.println("Por lo tanto Fermat tiene razon.");
		} else {
			System.out.println("No hay ninguna conbinacion. Por lo tanto Fermat tiene NO razon.");
		}
		
		System.out.println("\nPotencia de 3");
		if (validacionFermat(3, 1, 100)) {
			System.out.println("Por lo tanto Fermat  NO tiene razon.");
		} else {
			System.out.println("No hay ninguna conbinacion. Por lo tanto Fermat tiene razon.");
		}
		
		System.out.println("\nPotencia de 10");
		if (validacionFermat(10, 1, 100)) {
			System.out.println("Por lo tanto Fermat tiene razon.");
		} else {
			System.out.println("No hay ninguna conbinacion. Por lo tanto Fermat tiene razon.");
		}
	...
	// Lo comentado al final lo he descartado pq deduzco que "c" siempre tiene que
	// ser mayor que "b", y "b" a "a"(ya que 1^3 + 2^3 es lo mismo que 2^3 + 1^3)
	// para que haya posibilidad de que se cumpla la formula de Fermat.
	private static boolean validacionFermat(int iPotencia, int iMinimo, int iMaximo) {
		boolean booHayIgualdad = false;
	double dCoeficienteA = iMinimo, dCoeficienteB = dCoeficienteA + 1, dCoeficienteC = dCoeficienteB + 1, dMaximoC = Math.ceil(Math.pow((Math.pow(iMaximo - 1, 2) + (Math.pow(iMaximo - 1, 2))), 0.5)) ;
	
	do {
		do {
			do {
				
				if (Math.pow(dCoeficienteA, iPotencia) + Math.pow(dCoeficienteB, iPotencia) == Math.pow(dCoeficienteC, iPotencia)) {
					booHayIgualdad = true;
					System.out.println(dCoeficienteA+" elevado a "+iPotencia+" + "+dCoeficienteB+" elevado a "+iPotencia+" = "+dCoeficienteC+" elevado a "+iPotencia);
				}
				
				dCoeficienteC++;
//				if (dCoeficienteC == dMaximoC) {
//					dCoeficienteC = dCoeficienteB + 1;
//					if (dCoeficienteC == dCoeficienteA) {
//						dCoeficienteC++;
//					}
//				} else {
//					dCoeficienteC++;
...
//				}
			...
			} while (!booHayIgualdad && dCoeficienteC <= dMaximoC);

			dCoeficienteB++;
			dCoeficienteC = dCoeficienteB + 1;
//			if (dCoeficienteB == iMaximo) {
//				dCoeficienteB = dCoeficienteA + 1;
//				dCoeficienteC = dCoeficienteB + 1;
//				if (dCoeficienteC > iMaximo) {
//					dCoeficienteC = iMinimo;
//					if (dCoeficienteC == dCoeficienteA) {
//						dCoeficienteC++;
//					}
//				} else {
...
//					}
//				}
//			} SI ESTAS PENSANDO ESO, PARA QUE CARAJO SE COMPLICA TANTO LA VIDA.....

		} while (!booHayIgualdad && dCoeficienteB <= iMaximo);

		dCoeficienteA++;
		dCoeficienteB = dCoeficienteA + 1;
		dCoeficienteC = dCoeficienteB + 1;
		
	} while (!booHayIgualdad && dCoeficienteA <= iMaximo);

	return booHayIgualdad;
	}
