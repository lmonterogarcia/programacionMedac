CREATE DEFINER=`fotografo`@`%` PROCEDURE `reservafotografo`.`fotografo_sesion_search_by`(IN oObject JSON)
BEGIN
        ...
        IF vItems > 0 THEN 
            WHILE vIndex < vItems DO
                ...
                SET idSesionParam = JSON_UNQUOTE(JSON_EXTRACT(oObject, CONCAT('$[', vIndex, '].iIdSesion')));
               	...
               	SELECT * FROM Fotografo_Sesion WHERE dniFotografo = dniFotografoParam AND idSesion = idSesionParam;
