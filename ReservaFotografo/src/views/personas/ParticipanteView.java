...
public class ParticipanteView implements IPlantilla{
	...
	private static boolean create(Controller oCtrl) {
		String sDniContacto, sNombreContacto, sApellido1Contacto, sApellido2Contacto, sTelefonoContacto, setsEmailParticipante;
		...
		return oCtrl.addParticipante(new Participante(sDniContacto, sNombreContacto, sApellido1Contacto, sApellido2Contacto,
	...
	private static boolean update(Controller oCtrl) {
		...
		String sEmailParticipante = String.valueOf(Libreria.leer("Introduce un email", 1, 100, -1, -1, (byte) 6));
		Participante oParticipante = oCtrl.searchParticipante(new Participante(null, new Usuario(sEmailParticipante)));
		if (oParticipante != null) {
			oParticipante.setoUsuario(oCtrl.searchUsuario(oParticipante));
			if (oParticipante.getoUsuario() != null && oParticipante.checkParticipante()) {

				System.out.println("Modifica los datos basicos del participante: ");

				do {
					sDniContacto = String.valueOf(Libreria.leer("Introduce un dni (" + oParticipante.getsDniContacto() + ")",
				...
				do {
					sApellido2Contacto = String.valueOf(
							Libreria.leer("Introduce el segundo apellido (" + oParticipante.getsApellido2Contacto() + ")", 0,
									BMAXAPELLIDOS, -1, -1, (byte) 6));
				} while (!sApellido2Contacto.isEmpty() && sApellido2Contacto.length() > BMAXAPELLIDOS);
				...
				do {
					sTelefonoContacto = String.valueOf(Libreria.leer(
							"Introduce un telefono (" + oParticipante.getsTelefonoContacto() + ")", 0, 9, -1, -1, (byte) 6));
				} while (!sTelefonoContacto.isEmpty() && sTelefonoContacto.length() != 9);
				...
				if ((String.valueOf(Libreria.leer("¿Quiere modificar la fecha de nacimiento? (s/n) (" + sFechaNacimiento + ")",
					...
					do {
						try {
							...
							fLatitud = (float) (Libreria.leer("Introduce el segundo apellido", 0, BMAXAPELLIDOS, -1, -1,
									(byte) 5));
							fLongitud = (float) (Libreria.leer("Introduce el segundo apellido", 0, BMAXAPELLIDOS, -1,
									-1, (byte) 5));
	...
	private static Participante searchByEmail(Controller oCtrl) {
		String sEmailParticipante = String.valueOf(Libreria.leer("Introduce un email", 1, 100, -1, -1, (byte) 6));
		return oCtrl.searchParticipante(new Participante(null, new Usuario(sEmailParticipante)));
	}

	private static boolean remove(Controller oCtrl) {
		boolean bExito = false;
		Participante oParticipante = searchByEmail(oCtrl);
