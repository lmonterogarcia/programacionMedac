...
public class B6_EJ3 {
	...
	public static void main(String[] args) {
		...
		float[] aVector = { 2.6f, 4.1f, 7.13f };
		float fElemento = (float) leer("Introduce el numero a aniadir al vector ", -1, -1, 0, 50, (byte) 5);
		byte bPosicion = (byte) leer("Introduce en que posicion quiere añadir el elemento ", 0, aVector.length, -1, -1, (byte) 1);
		float[] aNuevoVector = aniadirElementoVector(aVector, fElemento, bPosicion);
		
		
		if (esElementoEnPosicionVector(aNuevoVector, bPosicion, fElemento)) {
			System.out.println("\nSe ha añadido correctamente el elemento a la posicion "+bPosicion);
		}
		System.out.println("Este es el resultado: "+imprimeVectorFloat(aNuevoVector));	
	}
	...
	private static float[] aniadirElementoVector(float[] aVector, float fElemento, int iPosicion) {
		float[] aNuevoVector = new float[aVector.length + 1];
		
		
		if (iPosicion > aVector.length) {
			iPosicion = aNuevoVector.length - 1;
		}
		
		for (int iContador = 0; iContador < aNuevoVector.length; iContador++) {
			
			if (iContador < iPosicion) {
				aNuevoVector[iContador] = aVector[iContador];
			} else if ((iContador == (iPosicion))) {
				aNuevoVector[iContador] = fElemento;
			} else {
				aNuevoVector[iContador] = aVector[iContador -1];
			}
	...
	private static boolean esElementoEnPosicionVector(float[] aVector, int iPosicion, float fElemento) {
		boolean booEscorrecto = false;
		
		if (aVector[iPosicion] == fElemento) {
			booEscorrecto = true;
		}
