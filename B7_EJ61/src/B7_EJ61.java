...
public class B7_EJ61 {

	public static void main(String[] args) {
		byte[][] aMatriz = {{1,2,3,4},{2,1,2,3},{3,2,1,2},{4,3,2,1}};
//		byte[][] aMatriz = {{2,2,3,4},{2,1,2,3},{3,2,1,2},{4,3,2,1}};
		
		System.out.println(mensajeFinal(esMatrizAntidiagonal(byteToIntMatriz(aMatriz))));
	...
	private static boolean esMatrizAntidiagonal(int[][] aMatriz) {
		boolean booEsAntidiagonal = true;
		int iContador = aMatriz.length - 1;

		if (aMatriz.length == aMatriz[0].length) {
			do {
				if (aMatriz[iContador][iContador] != 1) {
					booEsAntidiagonal = false;
				}
				iContador--;
			} while (booEsAntidiagonal && iContador >= 0);
		} else {
			booEsAntidiagonal = false;
		}

		return booEsAntidiagonal;
	}
	
	private static String mensajeFinal(boolean booEsAntidiagonal) {
		String sMensaje;
		
		if (booEsAntidiagonal) {
			sMensaje = "La matriz es antidiagonal";
		} else {
			sMensaje = "La matriz NO es antidiagonal";
		}
