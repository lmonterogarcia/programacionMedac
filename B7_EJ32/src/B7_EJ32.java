...
public class B7_EJ32 {

	public static void main(String[] args) {
		final byte BNUMEROSELEMENTOS = 10;
		byte[] aVector = new byte[BNUMEROSELEMENTOS];
		byte bOpcion, bPosicion;
		
		do {
			bOpcion = (byte) leer("MENU\n##################\n\n1. Visualizar.\n2. Insertar elemento al principio.\n3. Borrar elemento al principio.\n4. Insertar elemento al final.\n5. Borrar elemento al final.\n6. Insertar un elemento en una posición.\n7. Borrar un elemento de una posición.\n8. SALIR.\n\n", 1, 8, -1, -1, (byte) 1);
			...
			case 1:
				...
				break;
			case 2:
				if (!estaLleno(aVector)) {
					aVector = insertarPrincipio(aVector, (byte) leer("\nIntroduce el numero ", 1, 120, -1, -1, (byte) 1));
				} else {
...
//				System.out.println("\n"+visualizar(aVector));
				break;
			case 3:
				if (!estaVacio(aVector)) {
					aVector[0] = 0;
					aVector = intercambiarPosicionesDesdePrincipio(aVector, (byte) 0 , ultimaPosicionLlena(aVector));
				} else {
					System.err.println("\nImposible borrar datos, tabla vacia.");
				}
//				System.out.println("\n"+visualizar(aVector));
				break;
			case 4:
				if (!estaLleno(aVector)) {
					aVector[ultimaPosicionLlena(aVector) + 1] = (byte) leer("\nIntroduce el numero ", 1, 120, -1, -1, (byte) 1);
				} else {
					System.err.println("\nImposible introducir datos, tabla llena.");
				}
//				System.out.println("\n"+visualizar(aVector));
			...
			case 5:
				if (!estaVacio(aVector)) {
					aVector[ultimaPosicionLlena(aVector)] = 0;
				} else {
					System.err.println("\nImposible borrar datos, tabla vacia.");
				}
//				System.out.println("\n"+visualizar(aVector));
			...
			case 6:
				if (!estaLleno(aVector)) {
					bPosicion = (byte) leer("Introduce la posicion ", 1, (ultimaPosicionLlena(aVector) + 1), -1, -1, (byte) 1);
					aVector = intercambiarPosicionesDesdeFinal(aVector, (byte) (ultimaPosicionLlena(aVector) + 1), (byte) (bPosicion - 1 ));
					aVector[bPosicion - 1] = (byte) leer("\nIntroduce el numero ", 1, 120, -1, -1, (byte) 1);
				} else {
					System.err.println("\nImposible introducir datos, tabla llena.");
				}
//				System.out.println("\n"+visualizar(aVector));
			...
			case 7:
				if (!estaVacio(aVector)) {
					bPosicion = (byte) leer("Introduce la posicion ", 1, (ultimaPosicionLlena(aVector) + 1), -1, -1, (byte) 1);
					aVector[bPosicion - 1] = 0;
					aVector = intercambiarPosicionesDesdePrincipio(aVector, (byte)(bPosicion - 1), ultimaPosicionLlena(aVector));
				} else {
					System.err.println("\nImposible borrar datos, tabla vacia.");
				}
//				System.out.println("\n"+visualizar(aVector));
	...
	public static String visualizar(byte[] aVector) {
		String sResultado = "";
		for(int iContador = 0; iContador < aVector.length; iContador++) {
			
			if (aVector[iContador] == 0) {
				sResultado += "-";
			} else {
	...
	private static byte[] insertarPrincipio(byte[] aVector, byte bNumero) {
		aVector = intercambiarPosicionesDesdeFinal(aVector, ultimaPosicionLlena(aVector), (byte) 0);
		aVector[0] = bNumero;
		return aVector;
	}
	
	
	private static byte[] intercambiarPosicionesDesdeFinal(byte[] aVector, byte bPosicionUltimo, byte bPosicionHasta) {
		byte bTemporal;
	...
	private static byte[] intercambiarPosicionesDesdePrincipio(byte[] aVector, byte bDesde, byte bPosicionUltimo) {
		byte bTemporal;
	...
	private static byte ultimaPosicionLlena(byte[] aVector) {
...
//		}
	...
	private static boolean estaLleno(byte[] aVector) {
		boolean booEstaLleno = false;
		
		if (aVector[aVector.length - 1] != 0 ) {
			booEstaLleno = true;
		}
	...
	private static boolean estaVacio(byte[] aVector) {
		boolean booEstaVacio = true;
