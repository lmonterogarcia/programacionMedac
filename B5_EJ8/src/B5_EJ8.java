...
public class B5_EJ8 {
	...
	public static void main(String[] args) {
		byte[][] aMatriz = crearMatrizRandomByte(3, 3, (byte)1, (byte)50);
		
		System.out.println(imprimeMatrizByte(aMatriz) + "\nEl determinante de la matriz es "+(int)determinanteMatriz3(byteToIntMatriz(aMatriz)));
	...
	private static double determinanteMatriz3(int[][] aMatriz) {
		double dDeterminanteSuma = 1, dDeterminanteResta = 1, dDeterminante = 0;
		int iControlSuma, iControlResta;
		for (int iContadorTotal = 0; iContadorTotal < aMatriz.length; iContadorTotal++) {

			iControlSuma = iContadorTotal;
			iControlResta = (aMatriz.length - 1) - iContadorTotal;

			for (int iContador = 0; iContador < aMatriz.length; iContador++) {
				dDeterminanteSuma *= aMatriz[iContador][iControlSuma];

				if (iControlSuma == aMatriz.length - 1) {
					iControlSuma = 0;
				} else {
					iControlSuma++;
				}

				dDeterminanteResta *= aMatriz[iContador][iControlResta];

				if (iControlResta == 0) {
					iControlResta = aMatriz.length - 1;
				} else {
					iControlResta--;
				}
			}

			dDeterminante += dDeterminanteSuma - dDeterminanteResta;
			dDeterminanteSuma = 1;
			dDeterminanteResta = 1;
