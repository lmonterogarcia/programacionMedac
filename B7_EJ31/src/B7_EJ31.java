...
public class B7_EJ31 {

	public static void main(String[] args) {
		final byte BELEMENTOS = 50;
		byte [] aNumeros = crearVectorRandomByte(BELEMENTOS, (byte) -9, (byte) 9);

		System.out.println(imprimeVectorByte(aNumeros)+"\n\nLa Suma de los elementos con indice par es: "+sumaIndiceParVector(byteToIntVector(aNumeros))+"\nLa Suma de los elementos con indice impar es: "+sumaIndiceImparVector(byteToIntVector(aNumeros))+".");
	...
	//El 0 es par por definicion. (Según la diosa wikipedia).
	private static long sumaIndiceParVector(int[] aVector) {
		long lSuma = 0;
		int iElementos;
		
		if (aVector.length % 2 == 0) {
			iElementos = aVector.length - 2;
		} else {
			iElementos = aVector.length - 1;
		}
		
		for (int iContador = 0; iContador <= iElementos; iContador += 2) {

				lSuma += aVector[iContador]; 
	...
	private static long sumaIndiceImparVector(int[] aVector) {
		long lSuma = 0;
		int iElementos;
		...
		for (int iContador = 1; iContador <= iElementos; iContador += 2) {
