...
public class B7_EJ44 {

	public static void main(String[] args) {
//		char[] aMensaje = {H,O,L,A};
		char[] aMensaje = stringSimpleToCharVector((leer("Intrduce el mensaje", -1, 10, -1, -1, (byte) 10)).toString());

		System.out.println(rotacionCaracteres(aMensaje));
	}
	...
	private static String rotacionCaracteres(char[] aVector) {
		String sMensaje = charVectorToStringSimple(aVector)+", ";
		byte bPosiciones = 1;
		
		for (byte bContador = 0; bContador < aVector.length; bContador++) {
			sMensaje += charVectorToStringSimple(moverPosicionesVectorChar(aVector, (int) bPosiciones))+", ";
		}
		
		return sMensaje.substring(0, sMensaje.length() - 2)+".";
	}
	
	private static char[] moverPosicionesVectorChar(char[] aVector, int iPosiciones) {
		char cIntercambio, cIntercambio2;

		for (int iContador1 = 0; iContador1 < iPosiciones; iContador1++) {
			cIntercambio = aVector[aVector.length - 1];
			for (int iContador2 = aVector.length - 1; iContador2 > 0; iContador2--) {
				cIntercambio2 = aVector[iContador2];
				aVector[iContador2] = aVector[iContador2 - 1];
				aVector[iContador2 - 1] = cIntercambio2;
			}
			aVector[0] = cIntercambio;
		}
		cIntercambio = aVector[aVector.length - 1];
