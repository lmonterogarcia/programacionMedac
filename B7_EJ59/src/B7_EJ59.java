...
public class B7_EJ59 {

	public static void main(String[] args) {
		byte[] aNumeros = crearVectorRandomByte(18, (byte) 0, (byte) 9);

//		System.out.println(imprimeVectorByte(aNumeros));
		
		System.out.println(medianaVector(byteToIntVector(aNumeros)));
	...
	private static float medianaVector(int[] aVector) {
		float fMediana;
		int[] aVectorOrdenado = ordenarVectorBurbuja(aVector);
//		System.out.println(imprimeVectorInt(aVectorOrdenado));
		
		if (aVector.length % 2 == 0) {
			fMediana = (float) (aVectorOrdenado[(aVectorOrdenado.length / 2) - 1] + aVectorOrdenado[aVectorOrdenado.length / 2]) / 2;
		} else {
			fMediana = aVectorOrdenado[(int) (aVectorOrdenado.length / 2)];
		}
		 return fMediana;
	}
		...
		for (iContadorI = 1; iContadorI < iElementos; iContadorI++) {
			for (iContadorJ = (iElementos - 1); iContadorJ >= iContadorI; iContadorJ--) {
				if (aVectorOrdenar[iContadorJ] < aVectorOrdenar[iContadorJ - 1]) {
					...
					aVectorOrdenar[iContadorJ - 1] = (int) dTemporal;
				}
			}
		}
