...
public class B7_EJ45 {

	public static void main(String[] args) {

		// Funciona con el metodo mayusToMinus.
		char[] aMensaje = stringSimpleToCharVector((leer("Escribe tu mensaje: ", -1, -1, -1, -1, (byte) 9)).toString());

		System.out.println(charVectorToStringSimple(mayusToMinus(aMensaje)));
		
		// Funciona con el metodo mayusToMinus2.
//		String sMensaje = (leer("Escribe tu mensaje: ", -1, -1, -1, -1, (byte) 9)).toString();
//
//		System.out.println(mayusToMinus2(sMensaje));
	}
	...
	private static char[] mayusToMinus(char[] aVector) {
		char [] aVectorResultado = new char[aVector.length];
		
		for (int iContador = 0; iContador < aVector.length; iContador++) {
			if ((aVector[iContador] >= 'A' && aVector[iContador] <= 'Z') || aVector[iContador] == 'Ñ') {
				aVectorResultado[iContador] = Character.toLowerCase(aVector[iContador]);		
			} else {
				aVectorResultado[iContador] = aVector[iContador];
			}
		}
		return aVectorResultado;
	}
	
	private static String mayusToMinus2(String sMensaje) {
