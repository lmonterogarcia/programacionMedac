CREATE DEFINER=`fotografo`@`%` PROCEDURE `reservafotografo`.`lugar_update`(IN oObject JSON)
BEGIN
	...
	ELSE
        ...
        IF vItems > 0 THEN 
            ...
            WHILE vIndex < vItems DO
                ...
                -- CONTROLES E INSERT EN LAS SUBTABLAS || NO SE BORRAN LAS QUE QUEDAN LIBRE, SE VA A CREAR BASURA EN LA DB --
                -- SI DA TIEMPO SE MEJORARA PARA QUE NO DEJE BASURA EN LA DB --
            	...
            	googleMapLink = `googleMapLinkParam`,
           		latitud = `latitudParam`,
          		longitud = `longitudParam`,
        		calleLugar = `calleLugarParam`,
        		numeroLugar = `numeroLugarParam`,
       			referenciaCodigoPostal = `referenciaCodigoPostalParam`,
      			nombreLocalidad = `nombreLocalidadParam`,
     			nombreProvincia = `nombreProvinciaParam`,
    			nombrePais = `nombrePaisParam`
    			WHERE idLugar = idLugarParam;
