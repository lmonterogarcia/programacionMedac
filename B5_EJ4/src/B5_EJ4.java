...
public class B5_EJ4 {
	...
	public static void main(String[] args) {
		char[] aVectorPalindromo = objectToCharVector(leerVector("Introduce un caracter para la posicion ", 9, -1, -1, -1, -1, (byte) 7)); 

		if (esPalindromo(aVectorPalindromo)) {
			System.out.println("\n"+imprimeVectorSinComas(aVectorPalindromo) + " es un palindromo");
		} else {
			System.out.println("\n"+imprimeVectorSinComas(aVectorPalindromo) + " no es un palindromo");
		}
	}
	...
	public static boolean esPalindromo(char[] aCaracteres) {
		boolean booEsPalindromo = true;
		byte bContadorPrincipio = 0, bContadorFinal = (byte) (aCaracteres.length - 1);
		while (booEsPalindromo && bContadorFinal != 0) {
			if (aCaracteres[bContadorPrincipio] != aCaracteres[bContadorFinal]) {
				booEsPalindromo = false;
			}
			bContadorPrincipio++;
			bContadorFinal--;
		}
		return booEsPalindromo;
	}

	public static String imprimeVectorSinComas(char[] aVector) {
		String sResultado = "";
