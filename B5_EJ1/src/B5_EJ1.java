...
//		byte[] iTiemposDeCaida = leerVectorIncremento("Tiempo del objeto numero ", (int) 5, (int) 1, (int) 100);
//		byte[] iDiferenciaDeTiempos = calcularDiferencia(leerVectorIncremento("Tiempo del objeto numero ", (int) 5, (int) 1, (int) 100));
		System.out.println(
						"\nLa diferencia de tiempos es "
								+ imprimeVectorByte(calcularDiferenciaTiempos(
										leerTiempos("Tiempo del objeto numero ", (byte) 5, (byte) 1, (byte) 100)))
								+ ".");
	}
	...
	private static byte[] calcularDiferenciaTiempos(byte[] aVector) {
		byte bLongitudVector = (byte) aVector.length;
		byte[] iDiferencia = new byte[bLongitudVector];
		for (byte bContador = 0; bContador <= bLongitudVector - 1; bContador++) {
			iDiferencia[bContador] = (byte) (aVector[bContador] - aVector[0]);
		}

		return iDiferencia;
	}

	public static byte[] leerTiempos(String sMensaje, byte iTamanio, byte iMin, byte iMax) {

		byte iNumeroElementos = iTamanio, iControl = (byte) (iMin - 1);
		byte[] aVector = new byte[iNumeroElementos];

		for (int iContador = 0; iContador < aVector.length; iContador++) {
			aVector[iContador] = (byte) leer(sMensaje + (iContador + 1) + ": ", (long) iControl + 1, (long) iMax, -1, -1,
					(byte) 1);
