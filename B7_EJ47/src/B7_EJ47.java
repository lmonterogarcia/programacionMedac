...
public class B7_EJ47 {

	public static void main(String[] args) {
		String sFrase = (leer("Introduce el texto: ", -1, -1, -1, -1, (byte) 9)).toString();
		
		sFrase = mayusToMinus(sFrase);
		
		System.out.println(informeDeLetras(soloLetras(sFrase)));
	...
	private static char[] soloLetras(String sMensaje) {
		char[] aMensajeSoloLetras;
		String sSoloLetras= "";
		 
		for (int iContador = 0; iContador < sMensaje.length(); iContador++) {
			if (Character.isLetter(sMensaje.charAt(iContador))) {
				sSoloLetras += sMensaje.charAt(iContador);
			}
		}
		
		aMensajeSoloLetras = new char[sSoloLetras.length()];
		
		for (int iContador = 0; iContador < aMensajeSoloLetras.length; iContador++) {
			aMensajeSoloLetras[iContador] = sSoloLetras.charAt(iContador);
		}
		
		return aMensajeSoloLetras;
	}
	
	
	private static String informeDeLetras(char[] aLetras) {
		String sInforme = "";
		int iRepeticiones = 1;
		
		aLetras = ordenarVectorBurbuja(aLetras);
		
		for (int iContadorLetras = 0; iContadorLetras < aLetras.length; iContadorLetras++) {
			if (iContadorLetras == aLetras.length - 1) {
				if (aLetras[iContadorLetras] == aLetras[iContadorLetras - 1]) {
					sInforme += "\nLa letra " + aLetras[iContadorLetras] + " se repite " + iRepeticiones + " veces";
				} else {
					sInforme += "\nLa letra " + aLetras[iContadorLetras] + " se repite " + iRepeticiones + " vez";
				}
			} else {
				if (aLetras[iContadorLetras] == aLetras[iContadorLetras + 1]) {
					iRepeticiones++;
	...
	private static char[] ordenarVectorBurbuja(char aVector[]) {
		char cTemporal;
		char[] aVectorOrdenar = aVector;
		...
		for (iContadorI = 1; iContadorI < iElementos; iContadorI++) {
			for (iContadorJ = (iElementos - 1); iContadorJ >= iContadorI; iContadorJ--) {
				if (aVectorOrdenar[iContadorJ] < aVectorOrdenar[iContadorJ - 1]) {
					cTemporal = aVectorOrdenar[iContadorJ];
					aVectorOrdenar[iContadorJ] = aVectorOrdenar[iContadorJ - 1];
					aVectorOrdenar[iContadorJ - 1] = cTemporal;
				}
			}
		}
