...
public class B6_EJ2 {
	...
	public static void main(String[] args) {
		...
		byte [][] aMatriz = crearMatrizRandomByte(6, 6, (byte) 1, (byte) 9);
		
		System.out.println(imprimeMatrizByte(aMatriz));
		
		System.out.println(imprimeMatrizByte(floatToByteMatriz(ordenarMatrizDiagonales(byteToFloatMatriz(aMatriz)))));

		
//		float [][] aMatriz = crearMatrizRandomFloat(5, 5, (float) -9, (float) 9);
//		
//		System.out.println(imprimeMatrizFloat(aMatriz));
//		
//		System.out.println(imprimeMatrizFloat(ordenarMatrizDiagonales(aMatriz)));
	}
	...
	//ya que numeros reales son enteros y decimales de signo - y +, he utilizado float para crear la funcion.
	private static float[][] ordenarMatrizDiagonales(float[][] aMatriz) {

		for (int iContador = 0; iContador < aMatriz.length - 1; iContador++) {
			float fTemporal;
			
			if (iContador == 0) {
				
			//Ordena la diagonal principal
				for (int iContadorI = 1; iContadorI < aMatriz.length; iContadorI++) {
					for (int iContadorJ = (aMatriz.length - 1); iContadorJ >= iContadorI; iContadorJ--) {
						if (aMatriz[iContadorJ][iContadorJ] < aMatriz[iContadorJ - 1][iContadorJ - 1]) {
							fTemporal = aMatriz[iContadorJ][iContadorJ];
							aMatriz[iContadorJ][iContadorJ] = aMatriz[iContadorJ - 1][iContadorJ - 1];
							aMatriz[iContadorJ - 1][iContadorJ - 1] = fTemporal;
						}
					}
				}
			...
			//Ordena las diagonales secundarias hacia la derecha
				for (int iContadorI = 1; iContadorI <= aMatriz.length - iContador; iContadorI++) {
					for (int iContadorJ = aMatriz.length - 1; iContadorJ > iContador; iContadorJ--) {
						if (aMatriz[iContadorJ][iContadorJ - iContador] < aMatriz[iContadorJ - 1][iContadorJ - iContador  - 1]) {
							fTemporal = aMatriz[iContadorJ][iContadorJ - iContador];
							aMatriz[iContadorJ][iContadorJ - iContador] = aMatriz[iContadorJ - 1][iContadorJ - iContador  - 1];
							aMatriz[iContadorJ - 1][iContadorJ - iContador  - 1] = fTemporal;
						}
					}
				}
				
				//Ordena las diagonales secundarias hacia abajo
				for (int iContadorI = 1; iContadorI <= aMatriz.length - iContador; iContadorI++) {
					for (int iContadorJ = aMatriz.length - 1; iContadorJ > iContador; iContadorJ--) {
						if (aMatriz[iContadorJ - iContador][iContadorJ] < aMatriz[iContadorJ - iContador - 1][iContadorJ - 1]) {
							fTemporal = aMatriz[iContadorJ - iContador][iContadorJ];
							aMatriz[iContadorJ - iContador][iContadorJ] = aMatriz[iContadorJ - iContador - 1][iContadorJ - 1];
							aMatriz[iContadorJ - iContador - 1][iContadorJ - 1] = fTemporal;
						}
					}
				}
			}
	...
	private static float[] byteToFloatVector(byte[] aVector) {
	...
	private static byte[] floatToByteVector(float[] aVector) {
		byte[] aVectorCasteado = new byte[aVector.length];
		try {
			for (int iContador = 0; iContador < aVector.length; iContador++) {
				aVectorCasteado[iContador] = (byte) aVector[iContador];
	...
	private static float[][] byteToFloatMatriz(byte[][] aMatriz) {
	...
	private static byte[][] floatToByteMatriz(float[][] aMatriz) {
		byte[][] aMatrizCasteado = new byte[aMatriz.length][aMatriz[0].length];
		try {
			for (int iContadorFila = 0; iContadorFila < aMatriz.length; iContadorFila++) {
				for (int iContadorColumna = 0; iContadorColumna < aMatriz[iContadorFila].length; iContadorColumna++) {
...
//	Nueva funcion para Libreria Leer. Lee un double dentro de un rango sin llegar a los limites
	...
	private static int[] crearVectorRandomInt(int iTamanio, int iMin, int iMax) {
		...
		for (int iContador = 0; iContador < aVector.length; iContador++) {
			aVector[iContador] = (int) ((iMin - 1) + Math.floor(Math.random() * (iMax + 1) + 1));
		}
		return aVector;
	}
	...
	private static byte[][] crearMatrizRandomByte(int iFilas, int iColumnas, byte bMin, byte bMax) {
		...
		for (int iContadorFilas = 0; iContadorFilas < aMatriz.length; iContadorFilas++) {
			for (int iContadorColumnas = 0; iContadorColumnas < aMatriz[iContadorFilas].length; iContadorColumnas++) {
				aMatriz[iContadorFilas][iContadorColumnas] = (byte) ((bMin - 1) + Math.floor(Math.random() * (bMax - bMin + 1) + 1));
			}
		}
		return aMatriz;
	}
	private static short[][] crearMatrizRandomShort(int iFilas, int iColumnas, short shMin, short shMax) {
		...
		for (int iContadorFilas = 0; iContadorFilas < aMatriz.length; iContadorFilas++) {
			for (int iContadorColumnas = 0; iContadorColumnas < aMatriz[iContadorFilas].length; iContadorColumnas++) {
				aMatriz[iContadorFilas][iContadorColumnas] = (short) ((shMin - 1) + Math.floor(Math.random() * (shMax - shMin + 1) + 1));
			}
		}
		return aMatriz;
	}
	private static int[][] crearMatrizRandomInt(int iFilas, int iColumnas, int iMin, int iMax) {
		...
		for (int iContadorFilas = 0; iContadorFilas < aMatriz.length; iContadorFilas++) {
			for (int iContadorColumnas = 0; iContadorColumnas < aMatriz[iContadorFilas].length; iContadorColumnas++) {
				aMatriz[iContadorFilas][iContadorColumnas] = (int) ((iMin - 1) + Math.floor(Math.random() * (iMax - iMin + 1) + 1));
			}
		}
		return aMatriz;
	}
	private static long[][] crearMatrizRandomLong(int iFilas, int iColumnas, long lMin, long lMax) {
		...
		for (int iContadorFilas = 0; iContadorFilas < aMatriz.length; iContadorFilas++) {
			for (int iContadorColumnas = 0; iContadorColumnas < aMatriz[iContadorFilas].length; iContadorColumnas++) {
				aMatriz[iContadorFilas][iContadorColumnas] = (long) ((lMin - 1) + Math.floor(Math.random() * (lMax - lMin + 1) + 1));
			}
		}
		return aMatriz;
	}
