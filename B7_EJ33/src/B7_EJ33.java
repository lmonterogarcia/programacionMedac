...
public class B7_EJ33 {

	public static void main(String[] args) {
		String sFrase = (leer("Introduce una frase", -1, 25, -1, -1, (byte) 10)).toString();
//		String sFrase = "   El  Perro es   Blanco ";
		System.out.println(sFrase);
		System.out.println(eliminarEspaciosSobrantes(sFrase));
	}
	...
	//No veo la necesidad de utilizar un array.
	private static String eliminarEspaciosSobrantes(String sFrase) {
		String sMensaje = "";
		int iContadorPrincipio = 0;
		boolean booPrimeraLetra = false, booEspacio;

		do {
			if (sFrase.charAt(iContadorPrincipio) != ' ') {
				booPrimeraLetra = true;
				sMensaje += sFrase.charAt(iContadorPrincipio);
			}
			iContadorPrincipio++;

		} while (!booPrimeraLetra);

		for (int iContador = iContadorPrincipio; iContador < sFrase.length(); iContador++) {

			if (sFrase.charAt(iContador) != ' ') {
				sMensaje += sFrase.charAt(iContador);
			} else {
				booEspacio = true;
				iContador++;

				while (booEspacio && iContador < sFrase.length()) {
