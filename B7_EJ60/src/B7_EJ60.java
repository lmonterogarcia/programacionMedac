...
public class B7_EJ60 {

	public static void main(String[] args) {
		short shNumero = (short) leer("Introduce el numero N ", 1, 30000, -1, -1, (byte) 2), shContadorPrimos = 0;
		int iTotal = 0;
		float fMedia;
		
		for (short shContador = 1; shContador <= shNumero; shContador++) {
			if (esPrimo((int) shContador)) {
				shContadorPrimos++;
				iTotal += shContador;
			}
		}

		fMedia = (float) iTotal / shContadorPrimos;
		 if (Float.isNaN(fMedia)) {
			System.out.println("No hay media de los numeros primos desde 1 a "+shNumero);
		} else {
			System.out.println("La media de los numeros primos desde 1 a "+shNumero+" es: "+fMedia);
		}
