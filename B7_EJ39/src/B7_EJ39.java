...
public class B7_EJ39 {

	public static void main(String[] args) {
		short[] aAlumnos = { 1671, 68, 176, 45, 646, 817, 186, 8687, 6876, 86 };
		String[] aNombre = { "Pepe", "Jose", "Maria", "Marta", "Julia", "Julian", "Paco", "Luis", "Patricio", "Fernando" };
		float[] aNota = { 5.4f, 6.8f, 9.5f, 8.7f, 4.5f, 8.6f, 2.5f, 4.7f, 8.9f, 5.4f };
		byte bPosicion;
		short shCodigo;

		aNota = ordenarVectorBurbujaConReferencia(aAlumnos, aNota);
		aNombre = ordenarVectorBurbujaConReferencia(aAlumnos, aNombre);
		aAlumnos = ordenarVectorBurbuja(aAlumnos);
		
		do {
			
			shCodigo = (short) leer("\nIntroduce el codigo o pulsa 0 para salir ", 0, 9999, -1, -1, (byte) 2);

			if (shCodigo != 0) {
				bPosicion = (byte) busquedaDicotomica(shortToIntVector(aAlumnos), shCodigo);
				if (bPosicion != -1) {
					System.out.println(
							"\nNombre del alumno: " + aNombre[bPosicion] + "\nNota del alumon: " + aNota[bPosicion]);
				} else {
					System.out.println("\nNo existe ningun alumno con ese codigo.");
				}
			}
	...
	private static short[] ordenarVectorBurbuja(short aVector[]) {
		double dTemporal;
		short[] aVectorOrdenar = aVector;
		...
		for (iContadorI = 1; iContadorI < iElementos; iContadorI++) {
			for (iContadorJ = (iElementos - 1); iContadorJ >= iContadorI; iContadorJ--) {
				if (aVectorOrdenar[iContadorJ] < aVectorOrdenar[iContadorJ - 1]) {
					...
					aVectorOrdenar[iContadorJ - 1] = (short) dTemporal;
				}
			}
		}
	...
	private static float[] ordenarVectorBurbujaConReferencia(short[] aCodigo, float[] aNota) {
		double dTemporal;
		short[] aVectorReferencia = aCodigo;
		float[] aVector = aNota;
		...
		for (iContadorI = 1; iContadorI < iElementos; iContadorI++) {
			for (iContadorJ = (iElementos - 1); iContadorJ >= iContadorI; iContadorJ--) {
				if (aCodigo[iContadorJ] < aCodigo[iContadorJ - 1]) {
					...
					aCodigo[iContadorJ - 1] = (short) dTemporal;
					
					dTemporal = aVector[iContadorJ];
					aVector[iContadorJ] = aVector[iContadorJ - 1];
					aVector[iContadorJ - 1] = (float) dTemporal;
				}
			}
		}
	...
	private static String[] ordenarVectorBurbujaConReferencia(short[] aCodigo, String[] aNombre) {
		double dTemporal;
		String sTemporal;
		short[] aVectorReferencia = aCodigo;
		String[] aVector = aNombre;
		
		int iContadorI, iContadorJ, iElementos = aCodigo.length;

		for (iContadorI = 1; iContadorI < iElementos; iContadorI++) {
			for (iContadorJ = (iElementos - 1); iContadorJ >= iContadorI; iContadorJ--) {
				if (aVectorReferencia[iContadorJ] < aVectorReferencia[iContadorJ - 1]) {
				
					dTemporal = aVectorReferencia[iContadorJ];
					aVectorReferencia[iContadorJ] = aVectorReferencia[iContadorJ - 1];
					aVectorReferencia[iContadorJ - 1] = (short) dTemporal;
					
					sTemporal = aNombre[iContadorJ];
					aNombre[iContadorJ] = aNombre[iContadorJ - 1];
					aNombre[iContadorJ - 1] = sTemporal;
				}
			}
		}
	...
	private static int busquedaDicotomica(int[] aVector, int iBusqueda) {
		int iIzquierda = 0;
		int iDerecha = aVector.length - 1;
		int iCentro = (iIzquierda + iDerecha) / 2;
		int iPosicion;
		
		while ((iIzquierda <= iDerecha) && (aVector[iCentro] != iBusqueda)) {
			if (iBusqueda < aVector[iCentro]) {
				iDerecha = iCentro - 1;
			} else {
	...
	private static byte buscarCodigoDicotomica(short[] aVector, short shCodigo) {
		byte shPosicion = 0;
		boolean booEncontrado = false;
		
		do {
			if (aVector[shPosicion] == shCodigo) {
				booEncontrado = true;
			}
			
			shPosicion++;
		} while (!booEncontrado && shPosicion < aVector.length);
		
		if (!booEncontrado) {
			shPosicion = 0;
		}
		return (byte) (shPosicion -1);
	}
