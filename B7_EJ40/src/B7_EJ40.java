...
public class B7_EJ40 {

	public static void main(String[] args) {
		byte[] aVector = crearVectorRandomByte(100, (byte) 0, (byte) 9);
		String sMenores = "", sMayores = "";
		final byte BELEMENTOSENSENIAR= 10;
		
		aVector = ordenarVectorBurbuja(aVector);
		
		System.out.println(BELEMENTOSENSENIAR+" valores menores: "+menores(aVector, BELEMENTOSENSENIAR)+"\n"+BELEMENTOSENSENIAR+" valores mayores: "+mayores(aVector, BELEMENTOSENSENIAR));
	...
	private static String menores(byte[] aVector, byte bElementos) {
		String sMensaje = "";
		
		for (byte bContador = 0; bContador < bElementos; bContador++) {
			sMensaje += aVector[bContador]+", ";
		}
		
		
		
		return sMensaje.substring(0, sMensaje.length() - 2);
	}
	
	private static String mayores(byte[] aVector, byte bElementos) {
		String sMensaje = aVector[aVector.length - 1]+", ";
		boolean booEncontrado = false;
		byte bContador = (byte) (aVector.length - 2), bContadorMayores = 1, bNumero = aVector[aVector.length - 1];
		
		do {
			if (aVector[bContador] < bNumero) {
				sMensaje +=aVector[bContador]+", ";
				bNumero = aVector[bContador];
				bContadorMayores++;
			}
			
			if (bContadorMayores == bElementos) {
				booEncontrado = true;
			}
			
			bContador--;
		} while (!booEncontrado && bContador >= 0);
